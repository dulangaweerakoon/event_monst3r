...
def get_reconstructed_scene(args, outdir, model, device, silent, image_size, filelist, schedule, niter, min_conf_thr,
    ...
    if args.events:            
        if new_model_weights != args.weights:
            model = AsymmetricCroCo3DEventStereo.from_pretrained(new_model_weights).to(device)
    else:
        if new_model_weights != args.weights:
    ...
    if args.window_wise and args.prev_output_dir is not None:
        ...
        imgs = load_ev_images(filelist, size=image_size, verbose=not silent, dynamic_mask_root=None, fps=fps, num_frames=num_frames, imgs=prev_video_results['imgs'])
    else:
        prev_video_results = None
        imgs = load_ev_images(filelist, size=image_size, verbose=not silent, dynamic_mask_root=None, fps=fps, num_frames=num_frames)
