...
from dust3r.cloud_opt.base_opt import BasePCOptimizer, BaseEPCOptimizer, edge_str
from dust3r.cloud_opt.event_flow import EventContrastFlow
...
class EventGuidedPointCloudOptimizer(BaseEPCOptimizer):
    """ Optimize a global scene, given a list of pairwise observations.
    Graph node: images
    Graph edges: observations = (pred1, pred2)
    """
    ...
    #     flow_net = flow_net.to(device)
    #     flow_net.eval()
    #     # flow_net = EventContrastFlow(coarse_grid=(64, 64),iters=50,lr=0.1,lambda_var=0.05,blur_sigma=0.0,flow_clip=60.0).to(device)
    ...
    def get_flow(self, sintel_ckpt=False): #TODO: test with gt flow
        ...
        pair_events = [np.stack(self.events)[self._ei], np.stack(self.events)[self._ej]]

        # flow_net = load_RAFT() if sintel_ckpt else load_RAFT("third_party/RAFT/models/Tartan-C-T-TSKH-spring540x960-M.pth")
        # flow_net = flow_net.to(device)
        # flow_net.eval()
        flow_net = EventContrastFlow(coarse_grid=(256, 256),iters=50,lr=0.1,lambda_var=0.05,blur_sigma=1.0,flow_clip=60.0).to(device)
        ...
        if True:
            ...
            flow_ij = []
            flow_ji = []
            num_pairs = len(pair_events[0])
            for i in tqdm(range(0, num_pairs, chunk_size)):
                end_idx = min(i + chunk_size, num_pairs)
                events_ij = [torch.tensor(pair_events[0][i:end_idx]).float().to(device),
                        torch.tensor(pair_events[1][i:end_idx]).float().to(device)]
                # print(events_ij[0].shape, events_ij[1].shape)
                flow_ij.append(flow_net(events_ij[0].permute(0, 3, 1, 2), 
                                        events_ij[1].permute(0, 3, 1, 2)))
                flow_ji.append(flow_net(events_ij[1].permute(0, 3, 1, 2), 
                                        events_ij[0].permute(0, 3, 1, 2)))
                # imgs_ij = [torch.tensor(pair_imgs[0][i:end_idx]).float().to(device),
                #         torch.tensor(pair_imgs[1][i:end_idx]).float().to(device)]
                # flow_ij.append(flow_net(imgs_ij[0].permute(0, 3, 1, 2) * 255, 
                #                         imgs_ij[1].permute(0, 3, 1, 2) * 255, 
