...
from dust3r.utils.image import rgb, toevent
...
class BaseEPCOptimizer (nn.Module):
    """ Optimize a global scene, given a list of pairwise observations.
    Graph node: images
    Graph edges: observations = (pred1, pred2)
    """
    ...
    def _init_from_views(self, view1, view2, pred1, pred2,
        ...
        if 'event' in view1 and 'event' in view2:
            events = [torch.zeros((4,)+hw) for hw in self.imshapes]
            for v in range(len(self.edges)):
                idx = view1['idx'][v]
                events[idx] = view1['event'][v]
                idx = view2['idx'][v]
                events[idx] = view2['event'][v]
            self.events = toevent(events)
        # print(view1.keys())
        # print("EventS:", type(self.events[0]), type(self.imgs[0]))
