...
from dust3r.datasets.utils.transforms import ImgNorm, EventTransform
...
class BaseEventStereoViewDataset (EasyDataset):
    """ Define all basic options.
    ...
    def __init__(self, *,  # only keyword arguments
        ...
        self.ev_transform = ev_transform
        if isinstance(ev_transform, str):
            ev_transform = eval(ev_transform)
    ...
    def _crop_resize_if_necessary(self, image, depthmap, event, intrinsics, resolution, rng=None, info=None):
        """ This function:
            - first downsizes the image with LANCZOS inteprolation,
              which is better than bilinear interpolation in
        """
        ...
        r, b = cx + min_margin_x, cy + min_margin_y
        crop_bbox = (l, t, r, b)
        image, depthmap, intrinsics,event = cropping.crop_image_depthmap(image, depthmap, intrinsics, crop_bbox,event=event)
        ...
        if self.aug_focal:
            crop_scale = self.aug_focal + (1.0 - self.aug_focal) * np.random.beta(0.5, 0.5) # beta distribution, bi-modal
            image, depthmap, intrinsics,event = cropping.center_crop_image_depthmap(image, depthmap, intrinsics, crop_scale,event=event)

        if self.aug_crop > 1:
            target_resolution += rng.integers(0, self.aug_crop)
        image, depthmap, intrinsics,event = cropping.rescale_image_depthmap(image, depthmap, intrinsics, target_resolution,event=event) # slightly scale the image a bit larger than the target resolution
        # actual cropping (if necessary) with bilinear interpolation
        intrinsics2 = cropping.camera_matrix_of_crop(intrinsics, image.size, resolution, offset_factor=0.5)
        crop_bbox = cropping.bbox_from_intrinsics_in_out(intrinsics, intrinsics2, resolution)
        image, depthmap, intrinsics2,event = cropping.crop_image_depthmap(image, depthmap, intrinsics, crop_bbox,event=event)
        return image, depthmap, intrinsics2, event
