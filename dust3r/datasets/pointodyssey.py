...
from dust3r.datasets.base.base_stereo_view_dataset import BaseStereoViewDataset, BaseEventStereoViewDataset
...
class EventPointOdysseyDUSt3R(BaseEventStereoViewDataset):
    def __init__(self,
                 dataset_location='data/pointodyssey',
                 dset='train',
                 use_augs=False,
        ...
        for seq in self.sequences:
            ...
            info_path = os.path.join(seq, 'info.npz')
            annotations_path = os.path.join(seq, 'anno.npz')
            events_path = os.path.join(seq, 'events')
                ...
                if len(trajs_3d_shape) and trajs_3d_shape[1] > self.N:
                    ...
                    for stride in strides:
                        for ii in range(0,len(os.listdir(rgb_path))-self.S*max(stride,clip_step_last_skip)+1, clip_step):
                            ...
                            self.depth_paths.append([os.path.join(seq, 'depths', 'depth_%05d.png' % idx) for idx in full_idx])
                            self.normal_paths.append([os.path.join(seq, 'normals', 'normal_%05d.jpg' % idx) for idx in full_idx])
                            self.event_paths.append([os.path.join(seq, 'events', 'events_%05d.png' % idx) for idx in full_idx])
                            self.annotation_paths.append(os.path.join(seq, 'anno.npz'))
                            self.full_idxs.append(full_idx)
                            self.sample_stride.append(stride)
                        if self.verbose:
        ...
        # print the legths of the rgb_paths, depth_paths, event_paths, normal_paths
        print('len rgb_paths:', len(self.rgb_paths))
        print('len depth_paths:', len(self.depth_paths))
        print('len event_paths:', len(self.event_paths))
        print('len normal_paths:', len(self.normal_paths))
    ...
    def _get_views(self, index, resolution, rng):
        ...
        for i in range(2):
            ...
            if os.path.isfile(eventpath):
                event_img = cv2.imread(eventpath, cv2.IMREAD_GRAYSCALE)
                event_img = event_img.astype(np.float32) / 255.0
                # concatenate to rgb image
                # rgb_image = np.concatenate([rgb_image, event_img], axis=2)
            else:
                # find the closest event file in the same folder
                print("Couldn't find event file:", eventpath)
                event_folder = os.path.dirname(eventpath)
                event_files = glob.glob(os.path.join(event_folder, '*.png'))
                if len(event_files) > 0:
                    closest_event_file = min(event_files, key=lambda x: abs(int(x.split('_')[-1].split('.')[0]) - int(eventpath.split('_')[-1].split('.')[0])))
                    event_img = cv2.imread(closest_event_file, cv2.IMREAD_GRAYSCALE)
                    event_img = event_img.astype(np.float32) / 255.0
                    # concatenate to rgb image
                    # rgb_image = np.concatenate([rgb_image, event_img], axis=2)
...
class EventVoxelPointOdysseyDUSt3R(BaseEventStereoViewDataset):
    def __init__(self,
                 dataset_location='data/pointodyssey',
                 dset='train',
                 use_augs=False,
        ...
        for subdir in self.subdirs:
            for seq in glob.glob(os.path.join(subdir, "*/")):
                seq_name = seq.split('/')[-1]
                # check if the event_voxels folder exists
        ...
        for seq in self.sequences:
            ...
            info_path = os.path.join(seq, 'info.npz')
            annotations_path = os.path.join(seq, 'anno.npz')
            events_path = os.path.join(seq, 'event_voxels')
                ...
                if len(trajs_3d_shape) and trajs_3d_shape[1] > self.N:
                    ...
                    for stride in strides:
                        for ii in range(0,len(os.listdir(rgb_path))-self.S*max(stride,clip_step_last_skip)+1, clip_step):
                            ...
                            self.depth_paths.append([os.path.join(seq, 'depths', 'depth_%05d.png' % idx) for idx in full_idx])
                            self.normal_paths.append([os.path.join(seq, 'normals', 'normal_%05d.jpg' % idx) for idx in full_idx])
                            self.event_paths.append([os.path.join(seq, 'event_voxels', 'voxel_%05d.npy' % idx) for idx in full_idx])
                            self.annotation_paths.append(os.path.join(seq, 'anno.npz'))
                            self.full_idxs.append(full_idx)
                            self.sample_stride.append(stride)
                        if self.verbose:
...
if __name__ == "__main__":
    ...
    dataset = EventVoxelPointOdysseyDUSt3R(
