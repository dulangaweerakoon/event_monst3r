...
from dust3r.model import AsymmetricCroCo3DStereo, AsymmetricCroCo3DEventStereo, VGGTEventRGBStereo, EventVGGTStereoV1
from dust3r.image_pairs import make_pairs
from dust3r.utils.image import load_images, load_prev_video_results, rgb, enlarge_seg_masks, load_ev_voxels, load_ev_images
...
def get_args_parser():
    ...
    parser.add_argument('--events', action='store_true', default=False, help='Use events as input')
...
def get_reconstructed_scene(args, outdir, model, device, silent, image_size, filelist, schedule, niter, min_conf_thr,
    ...
    if args.events:            
        if new_model_weights != args.weights:
            model = EventVGGTStereoV1(pos_embed='RoPE100', patch_embed_cls='ManyAR_PatchEmbed', img_size=(512, 512), head_type='dpt', output_mode='pts3d', depth_mode=('exp', -inf, inf), conf_mode=('exp', 1, inf), enc_embed_dim=1024, enc_depth=24, enc_num_heads=16, dec_embed_dim=768, dec_depth=12, dec_num_heads=12, freeze='encoder').from_pretrained(new_model_weights).to(device)
    else:
        if new_model_weights != args.weights:
    ...
    if args.window_wise and args.prev_output_dir is not None:
        ...
        imgs = load_ev_voxels(filelist, size=image_size, verbose=not silent, dynamic_mask_root=None, fps=fps, num_frames=num_frames, imgs=prev_video_results['imgs'])
    else:
        prev_video_results = None
        imgs = load_ev_voxels(filelist, size=image_size, verbose=not silent, dynamic_mask_root=None, fps=fps, num_frames=num_frames)
    ...
    if len(imgs) > 2:
        if args.events:
            mode = GlobalAlignerMode.EventGuidedPointCloudOptimizer
        else:
    ...
    if mode == GlobalAlignerMode.PointCloudOptimizer or mode == GlobalAlignerMode.EventGuidedPointCloudOptimizer:
        if args.window_wise:
...
if __name__ == '__main__':
    ...
    model = AsymmetricCroCo3DEventStereo.from_pretrained(weights_path).to(args.device)
